    ...
    for (const auto &seed : seeds) {
        ...
        for (int depth = 0; depth < 5 && !base.empty(); ++depth) {
            for (const auto &relative : candidates) {
                ...
                if (std::filesystem::exists(file, ec) && !ec) return file;
            }
...
void LoadAssets(AppState *state) {
    ...
    if (!bg.empty()) {
        ...
        if (bitmap->GetLastStatus() == Gdiplus::Ok) state->background_bitmap = std::move(bitmap);
    }
    ...
    if (!icon.empty()) {
        ...
        if (bitmap->GetLastStatus() == Gdiplus::Ok) state->icon_bitmap = std::move(bitmap);
    }
    ...
    for (const auto &candidate : fonts) {
        ...
        if (collection->AddFontFile(file.c_str()) != Gdiplus::Ok) continue;
...
HitTarget HitTest(const AppState *state, POINT point) {
    const Layout layout = ComputeLayout(state);
    if (!state->processing && ContainsPoint(layout.choose_button, point)) return HitTarget::choose_button;
    if (!state->last_output_dir.empty() && ContainsPoint(layout.output_button, point)) return HitTarget::output_button;
...
void DrawImageWithOpacity(Gdiplus::Graphics &g, Gdiplus::Image *image,
    ...
    matrix.m[0][0] = 1.0f; matrix.m[1][1] = 1.0f; matrix.m[2][2] = 1.0f; matrix.m[4][4] = 1.0f;
...
void DrawUiFlat(Gdiplus::Graphics &g, const AppState *state, const Layout &layout) {
    ...
    // status pill (top-right)
    {
        ...
        if (state->has_result && !state->last_run_success) dot_color = Gdiplus::Color(255, 230, 90, 90);
        ...
        Gdiplus::StringFormat fmt; fmt.SetFormatFlags(Gdiplus::StringFormatFlagsNoWrap);
        ...
        Gdiplus::GraphicsPath pill_path; AddRoundedRect(pill_path, Gdiplus::RectF(pill_x, pill_y, pill_w, pill_h), pill_h * 0.5f);
        g.FillPath(&pill_bg, &pill_path);
        Gdiplus::Pen pill_pen(kHair, 1.0f); g.DrawPath(&pill_pen, &pill_path);
    ...
    Gdiplus::GraphicsPath drop_path; AddRoundedRect(drop_path, layout.drop_zone, 14.0f);
    ...
    auto DrawPrimary = [&](const Gdiplus::RectF &rect, const std::wstring &text, bool hover, bool pressed, bool enabled) {
        Gdiplus::GraphicsPath path; AddRoundedRect(path, rect, rect.Height * 0.5f);
    ...
    {
        std::wstring p_text;
        if (state->total > 0) p_text = std::to_wstring(state->completed) + L" / " + std::to_wstring(state->total);
        else p_text = L"—";
    ...
    for (int index = 0; index < visible; ++index) {
        int li = start + index;
        if (li >= static_cast<int>(state->logs.size())) break;
        const auto &line = state->logs[li];
        Gdiplus::Color tc = kInkSoft;
        if (line.find(L"[FAILED]") != std::wstring::npos) tc = Gdiplus::Color(255, 220, 80, 80);
        else if (line.find(L"[DONE]") != std::wstring::npos) tc = Gdiplus::Color(255, 90, 160, 110);
        else if (line.find(L"[START]") != std::wstring::npos) tc = kAccent;
...
void PaintUi(AppState *state, HDC hdc) {
    RECT rc{}; GetClientRect(state->window, &rc);
...
void StartPipeline(AppState *state, const std::vector<std::filesystem::path> &inputs, bool auto_close) {
    ...
    if (state->processing) { AppendLog(state, L"[BUSY] 当前有解包工作正在进行，请稍候。"); return; }
...
std::vector<std::filesystem::path> PickArcFiles(HWND owner) {
    ...
    if (!GetOpenFileNameW(&dialog)) return files;
    const std::wstring directory = buffer.data();
    const wchar_t *cursor = buffer.data() + directory.size() + 1;
    if (*cursor == L'\0') { files.emplace_back(directory); return files; }
    while (*cursor != L'\0') { files.emplace_back(std::filesystem::path(directory) / cursor); cursor += std::wcslen(cursor) + 1; }
    return files;
}

void OpenOutputDirectory(AppState *state) {
    if (state->last_output_dir.empty()) return;
...
AutoRunConfig ParseCommandLine() {
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        if (arg == L"--auto-close") config.auto_close = true;
        else config.inputs.emplace_back(arg);
    }
    LocalFree(argv);
    return config;
}
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam) {
    ...
    case WM_MOUSEMOVE:
        if (state) {
            ...
            if (target != state->hot_target) { state->hot_target = target; InvalidateRect(hwnd, nullptr, FALSE); }
        ...
        return 0;
    case WM_MOUSELEAVE:
        if (state) { state->mouse_tracking = false; state->hot_target = HitTarget::none; InvalidateRect(hwnd, nullptr, FALSE); }
        return 0;
    case WM_SETCURSOR:
    ...
    case WM_LBUTTONDOWN:
        if (state) {
            ...
            if (state->pressed_target != HitTarget::none) { SetCapture(hwnd); InvalidateRect(hwnd, nullptr, FALSE); }
        }
        return 0;
    case WM_LBUTTONUP:
        if (state) {
            ...
            if (released == pressed) {
                if (pressed == HitTarget::drop_zone || pressed == HitTarget::choose_button) StartPipeline(state, PickArcFiles(hwnd), false);
                else if (pressed == HitTarget::output_button) OpenOutputDirectory(state);
            }
        }
        return 0;
    case WM_MOUSEWHEEL:
    ...
    case kMessageLog:
        if (state) {
            ...
            if (payload) AppendLog(state, *payload);
        }
        return 0;
    case kMessageProgress:
        if (state) {
            ...
            if (payload) {
                state->completed = payload->completed; state->total = payload->total;
    ...
    case kMessageFinished:
        if (state) {
            ...
            if (payload) {
                state->processing = false; state->has_result = true;
                if (state->worker.joinable()) state->worker.join();
                state->last_run_success = payload->success; state->last_output_dir = payload->output_dir;
                if (payload->success && state->total > 0) state->completed = state->total;
                ...
                if (payload->auto_close && payload->success) PostMessageW(hwnd, WM_CLOSE, 0, 0);
                InvalidateRect(hwnd, nullptr, FALSE);
            }
        }
        return 0;
    case WM_PAINT:
        if (state) { PAINTSTRUCT ps{}; HDC hdc = BeginPaint(hwnd, &ps); PaintUi(state, hdc); EndPaint(hwnd, &ps); return 0; }
        break;
    case WM_CLOSE:
        if (state && state->processing) return 0;
        DestroyWindow(hwnd);
        return 0;
    case WM_DESTROY:
        if (state) {
            if (state->worker.joinable()) state->worker.join();
            state->private_fonts.reset(); state->background_bitmap.reset(); state->icon_bitmap.reset();
            if (state->gdiplus_token) Gdiplus::GdiplusShutdown(state->gdiplus_token);
        }
        PostQuitMessage(0);
        return 0;
    }
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, PWSTR, int show_command) {
    ...
    if (!auto_run.inputs.empty()) StartPipeline(&state, auto_run.inputs, auto_run.auto_close);
