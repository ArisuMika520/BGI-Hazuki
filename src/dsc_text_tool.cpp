...
namespace hazuki::dsc
{
    namespace
    {
        ...
        public:
            ...
            {
                EnsureAvailable(4);
                const auto value = static_cast<std::uint32_t>(data_[position_]) | (static_cast<std::uint32_t>(data_[position_ + 1]) << 8) | (static_cast<std::uint32_t>(data_[position_ + 2]) << 16) | (static_cast<std::uint32_t>(data_[position_ + 3]) << 24);
                position_ += 4;
                return value;
            }
        ...
        bool StartsWith(const std::vector<std::uint8_t> &bytes, const char *literal)
        {
            ...
            return bytes.size() >= length && std::equal(reinterpret_cast<const std::uint8_t *>(literal), reinterpret_cast<const std::uint8_t *>(literal) + length, bytes.begin());
        }
        ...
        std::vector<std::uint8_t> EncodeScriptText(const std::wstring &text, std::uint32_t codepage)
        {
            ...
            {
                if (text[i] == L'&' && i + 5 < text.size() && text[i + 1] == L'#' && IsHexDigit(text[i + 2]) && IsHexDigit(text[i + 3]) && IsHexDigit(text[i + 4]) && IsHexDigit(text[i + 5]))
                {
                    flush_chunk();
                    const std::uint16_t value = static_cast<std::uint16_t>(
                        (HexValue(text[i + 2]) << 12) | (HexValue(text[i + 3]) << 8) | (HexValue(text[i + 4]) << 4) | HexValue(text[i + 5]));
                    if ((value >> 8) != 0)
                    {
        ...
        {
            return ((static_cast<std::uint32_t>(data[position]) << 16) ^ (static_cast<std::uint32_t>(data[position + 1]) << 8) ^ static_cast<std::uint32_t>(data[position + 2])) & static_cast<std::uint32_t>(kMatchHashSize - 1);
        }
        ...
        TextKind ParseKind(const std::wstring &value)
        {
            ...
            if (value == L"ruby_kanji")
                return TextKind::RubyKanji;
            if (value == L"ruby_furigana")
                return TextKind::RubyFurigana;
            if (value == L"backlog")
                return TextKind::Backlog;
            if (value == L"file")
                return TextKind::File;
    ...
    bool IsDscScript(const std::filesystem::path &path)
    {
        ...
        return file.gcount() == static_cast<std::streamsize>(buffer.size()) && std::equal(kDscMagic.begin(), kDscMagic.begin() + 16, buffer.begin());
    }
